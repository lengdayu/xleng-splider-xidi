...
async function getMainHtml() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //创建logo文件夹
      if (!fs.existsSync(`${currentDir}/logo`)) {
        fs.mkdirSync(`${currentDir}/logo`);
      }
      downloadFile(
        mainHtml.icons.topLogoUrl,
        `${currentDir}/logo`,
        "logoImg.png"
      );
      ...
      //2.3获取商品列表
      //创建商品icons文件夹
      if (!fs.existsSync(`${currentDir}/shoppIcons`)) {
        fs.mkdirSync(`${currentDir}/shoppIcons`);
      }
      ...
      for (let i of firstClassify) {
        ...
        for (let j of secClassify) {
          ...
          downloadFile(
            secClassifyInfo.iconUrl,
            `${currentDir}/shoppIcons`,
            `${secClassifyInfo.title}.png`
          );
      ...
      //2.4获取轮播图图片
      //创建轮播图文件夹
      if (!fs.existsSync(`${currentDir}/banner`)) {
        fs.mkdirSync(`${currentDir}/banner`);
      }
      ...
      //2.5专题精选
      //创建专题文件夹
      if (!fs.existsSync(`${currentDir}/recommend`)) {
        fs.mkdirSync(`${currentDir}/recommend`);
      }
      let recommendInfo = {
        title: "",
        item: [],
      ...
      for (let i of recommnedItem) {
        ...
        downloadFile(
          await i.findElement(By.css("img")).getAttribute("src"),
          `${currentDir}/recommend`,
          `${await i.findElement(By.css(".text")).getText()}.png`
        );
      }
      ...
      await driver.quit();
      resolve(mainHtml.shoppClassifyInfo);
    } catch (error) {
      reject(error);
    }
  });
