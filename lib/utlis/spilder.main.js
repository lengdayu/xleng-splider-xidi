...
async function getMainHtml() {
  ...
  for (let i of topCommodityItems) {
    let itemInfo = {};
    itemInfo.title = await i.findElement(By.css(".title>span")).getText();
  ...
  while (startIndex <= endIndex) {
    ...
    //滚动到当前区域
    await driver.actions().scroll(0, 0, 0, 0, specialCommodityItem).perform();
    ...
    let specialCommodityItemChildren = await specialCommodityItem.findElements(
      By.css(".homepro-item")
    );
    for (let i of specialCommodityItemChildren) {
      specialCommodityItemInfo.items.push({
        imgUrl: await i.findElement(By.css(".pic>a>img")).getAttribute("src"),
        desc: await i.findElement(By.css(".pic>a>img")).getAttribute("title"),
  ...
  let countryIconItems = await countryIcon.findElements(By.css(".stroll-item"));
